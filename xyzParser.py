...
for chart_id in chart_ids:
    ...
    for user in users:
        if user not in incomplete_users:
            ...
            xyY_target = get_chromaticity(xyz)
            row_array = [f"user_{(users.index(user))}", colorblinds[user], xyY_target["x"], xyY_target["y"]]
            output_rows.append(row_array)
    filename = os.path.join(path, DEST_FOLDER, f"{chart_id}.csv")
    os.makedirs(os.path.dirname(filename), exist_ok=True)
    with open(filename, 'w', encoding='UTF8', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(["user", "is_colorblind", "x", "y"])
        writer.writerows(output_rows)

colors_csv_rows = [["chart_id", "file_path", "target_x", "target_y", "background_x", "background_y"]]
with open(os.path.join(path, "colors.csv"), mode='r') as csv_file:
    ...
    for row in csv_reader:
        ...
        xyY_target = get_chromaticity(rgb2XYZ(row["first_foreground"]))
        if row["background_color"] == "wn" :
            xyY_background["x"] = xyY_background["y"] = xyY_background["Y"] = "NaN"
        else:
            xyY_background = get_chromaticity(rgb2XYZ(row["background_color"]))
        colors_csv_rows.append([
            row["chart_id"],
            filepath,
            xyY_target["x"], xyY_target["y"],
            xyY_background["x"], xyY_background["y"]
        ])
